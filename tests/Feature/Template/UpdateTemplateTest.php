...
class UpdateTemplateTest extends TestCase
{
    ...
    public function test_it_should_update_the_template(): void
    {
        ...
        $response = $this->actingAs($user)->putJson("/api/template/{$template->id}",[
    ...
    public function test_it_should_return_401_when_user_is_not_authenticated():void
    {
        ...
        $response = $this->putJson("/api/template/{$template->id}",[
            'name' => $this->faker()->realText(100),
            'description' => $this->faker()->realText(),
            rand(0,1) == 1 && 'image' => $file,
        ]);
    ...
    public function test_it_should_return_404_when_providing_unexisting_template():void
    {
        ...
        $response = $this->actingAs($user)->putJson("/api/template/200",[
            'name' => $this->faker()->realText(100),
            'description' => $this->faker()->realText(),
            rand(0,1) == 1 && 'image' => $file,
        ]);
